...
首次真机核对里 11 个 `MISSING_IN_INSTANCE` 全是**设计如此**的条目：本机专用运维脚本、纯文档、记录在册的实例资产。它们每次都出现，等于给核对结果加了恒定噪声 —— 而噪声的代价是真正的回归被淹没。

- 新增 `--ignore <file>`，默认读取 `<root>/.fingerprint-ignore`（一行一个路径，`/` 结尾表示整个目录，`#` 为注释）
- 忽略清单文件自身永远忽略，避免把自己报成差异
- 结论行与末尾各打印一次忽略命中数，忽略了什么一目了然

验证（合成实例指纹，含 `.env` + 3 个第三方主题文件 + 3 个本机专用文件，**外加 1 个对照用的真实热修** `core/routes/__server-hotfix.js`）：

```
需回收 1 · 内容不同 0 · 仓库独有 0 · 忽略清单 12 条（命中 16 个实例文件）
  MISSING_IN_REPO  core/routes/__server-hotfix.js   ← 对照项照样被报出来
```

即：忽略清单不会掩盖真实发现，只是把"设计如此"的部分隐去。

### 🐛 第 4 次同一类事故：`content/themes/default/partials/footer.html` 也漏在同步清单外

部署前检查时发现：即使把 footer 回收入库，**同步脚本的清单里也没有它** —— 同步不会推它，差异永远归不了零。

这已经是同一个根因的第 4 次发作：

| # | 文件 | 表现 |
|---|---|---|
| 1 | `core/utils/tag-cloud-utils.js` | 漏出清单 → 标签别名只在部分路由生效（"看起来像没生效"） |
| 2 | `core/admin/static/js/table/modules/tabulator.js` | 只在服务器上有修复，仓库缺失（漏出清单 → 推不出去也拉不回来） |
| 3 | `content/themes/default/partials/footer.html` | 同上，服务器与仓库不一致 |
| 4 | 同 2、3 | 入库后**仍然**不在清单里，同步依旧推不过去 |

结论：**同步清单是这套部署方式的单点**。脚本里的清单自检（`-ManifestCheckDays`）默认是警告模式，建议在发版流程里改成 `-StrictManifest`，或者干脆每次同步前都用指纹工具比一次 —— 后者能同时发现"清单漏了"和"服务器上有手改"两类问题。
