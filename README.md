...
> 实测数据、A–E 交付清单与「学段维度 / 演示内容处理」两个待决方案的完整说明见 **`TAG-GOVERNANCE.md`**。
...
- **别名归一（不改内容文件）**：`content/data/tag-aliases.json` 把「同一个概念的不同写法」在**读取时**合并——标签云、标签页（旧链接 301 跳到规范标签）、公开 API 的标签字段与 `?tag=` 过滤、站内搜索的标签面、sitemap/RSS 全部生效；删掉该文件即完全回滚。
  ```json
  {
  ...
  ```
  - `aliases`：别名 → 规范名，可链式（`a`→`b`→`c`，内部有环检测），键名按归一化后**忽略大小写**匹配
  - `drop`：整条丢弃（例如上游演示文章带来的噪声标签），仅影响展示与筛选，内容文件不动
...
学段是**独立字段**（不属于标签），避免「小学/初中/高中」把标签云变成既不像分类也不像关键词的混合体：

- frontmatter 写入 `stage: "小学"`（单值；归一化规则与标签一致，`初 中` 与 `初中` 视为同一个学段）
- **`/stage` 总览页** 与 **`/stage/<学段>` 列表页**（复用当前主题的分类页模板，卡片与标签页完全一致；顶部「按学段浏览」筛选条由全局钩子注入，**不需要改主题**）
- **文章卡片与文章页显示学段徽标**（`🎓 小学`，点击进入该学段列表）；非规范写法（如 `/stage/小 学`）会 **301** 到规范地址
- **公开 API**：`/api/public/posts?stage=小学`，条目里带 `stage` 字段；`GET /api/stages` 返回学段清单与篇数
- **后台编辑器**新增「学段」输入框（带既有学段自动补全）
- **站内搜索**把学段纳入打分（搜「小学」能命中该学段文章，命中提示会显示「学段」）
- 迁移：把已经当作标签使用的学段搬到字段里（带备份、可整批回滚）
  ```bash
  ...
  ```
  迁移只改 `tags:` 与 `stage:` 两行，其余字节不变；已有 `stage` 的文章**不会被覆盖**。
